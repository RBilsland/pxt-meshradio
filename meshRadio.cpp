...
SINGLETON(MeshRadioWrap);
CODAL_RADIO* getMeshRadio() {
    auto wrap = getMeshRadioWrap();
    if (NULL != wrap)
        return wrap->getMeshRadio();    
    return NULL;
}
#endif // #else

    bool meshRadioEnabled = false;
    bool init = false;
    int meshRadioEnable() {
#ifdef CODAL_RADIO
        auto meshRadio = getRadio();
        if (NULL == meshRadio) 
        ...
        int r = meshRadio->enable();
        if (r != DEVICE_OK) {
            target_panic(43);
            return r;
        }
        if (!init) {
            getMeshRadio()->setGroup(0); //Default group zero. This used to be pxt::programHash()
            getMeshRadio()->setTransmitPower(6); // start with high power by default
            init = true;
        }
        meshRadioEnabled = true;
        return r;
#else
        return DEVICE_NOT_SUPPORTED;
#endif
    ...
    */
...
#ifdef CODAL_RADIO        
        if (meshRadioEnable() != DEVICE_OK) return;

        getMeshRadio()->event.eventReceived(CODAL_EVENT(src, value, CREATE_ONLY));
#endif        
    }
...
#ifdef CODAL_RADIO        
        if (meshRadioEnable() != DEVICE_OK) return NULL;

        auto p = getMeshRadio()->datagram.recv();
#if CODAL_RADIO_MICROBIT_DAL
    ...
    /**
     * Internal use only. Sends a raw packet through the radio (assumes RSSI appended to packet)
     */
    //% async
    void sendRawPacket(Buffer msg) {
#ifdef CODAL_RADIO        
        if (meshRadioEnable() != DEVICE_OK || NULL == msg) return;

        // don't send RSSI data; and make sure no buffer underflow
        int len = msg->length - sizeof(int);
        if (len > 0)
            getMeshRadio()->datagram.send(msg->data, len);
#endif            
    }
    ...
    void onDataReceived(Action body) {
        ...
        getMeshRadio()->datagram.recv(); // wake up read code
#endif       
    }
    ...
    */
    ...
    void setTransmitPower(int power) {
        ...
        getMeshRadio()->setTransmitPower(power);
#endif        
    }
...
#ifdef CODAL_RADIO        
        if (meshRadioEnable() != DEVICE_OK) return;
        getMeshRadio()->setFrequencyBand(band);
#endif        
    }
}
