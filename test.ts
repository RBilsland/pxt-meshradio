...
function initSender() {
            ...
            for (let i = 0; i < TEST_COUNT; i++) {
                ...
                while (lastAck !== testIndex) {
                    if (stage === TestStage.Integer || stage === TestStage.Double) {
                        meshRadio.sendNumber(lastReceived)
                    }
                    else if (stage === TestStage.IntValue || stage === TestStage.DblValue) {
                        meshRadio.sendValue(lastString, lastReceived)
                    }
                    else if (stage === TestStage.String) {
                        meshRadio.sendString(lastString);
                    }
                    else if (stage === TestStage.Buffer) {
                        meshRadio.sendBuffer(lastBuf);
                    }
                    basic.pause(10);
                }
                testIndex++;
    ...
    meshRadio.onReceivedNumber(function (receivedNumber: number) {
...
let lastPacket = new meshRadio.Packet();
let currentPacket = new meshRadio.Packet();
...
function initReceiver() {
    ...
    meshRadio.onDataReceived(function () {
        meshRadio.receiveNumber();

        currentPacket.receivedNumber = meshRadio.receivedNumber();
        currentPacket.receivedString = meshRadio.receivedString();
        currentPacket.receivedBuffer = meshRadio.receivedBuffer();
        ...
        switch (stage) {
            case TestStage.Integer:
                verifyInt(meshRadio.receivedNumber());
                break;
            case TestStage.Double:
                verifyDouble(meshRadio.receivedNumber());
                break;
            case TestStage.IntValue:
                verifyIntValue(meshRadio.receivedString(), meshRadio.receivedNumber());
                break;
            case TestStage.DblValue:
                verifyDblValue(meshRadio.receivedString(), meshRadio.receivedNumber());
                break;
            case TestStage.String:
                verifyString(meshRadio.receivedString());
                break;
            case TestStage.Buffer:
                verifyBuffer(meshRadio.receivedBuffer());
                break;
        }
    })
    ...
    control.inBackground(function () {
        while (running) {
            meshRadio.sendNumber(testIndex);
            basic.pause(10)
        }
    })
}
